 ...
 */
export function BulkSelect({ total, name = "unit_ids", label = "Select bikes for a bulk action" }: { total: number; name?: string; label?: string }) {
  const [count, setCount] = useState(0);
  useEffect(() => {
    const boxes = () => Array.from(document.querySelectorAll<HTMLInputElement>(`input[name="${name}"]`));
  ...
  const toggleAll = (checked: boolean) => {
    document.querySelectorAll<HTMLInputElement>(`input[name="${name}"]`).forEach((b) => { b.checked = checked; });
  ...
  return (
    <label className="flex items-center gap-2 text-sm">
      ...
      <span className="text-muted">{count === 0 ? label : `${count} selected`}</span>
    </label>
  );
}
