...
**Parts & accessories (2026-09-05):** `orders.kind` / `units.kind` = `bike` | `parts`. Parts special-order lines from Lightspeed become `parts` orders (one per line, `SO<lineID>`, listed per customer on the **Parts** tab, `/app/parts`). They are invited like bikes (one text per customer listing all items), the customer books **any future slot** (≥ 1 h ahead; no capacity, concurrency, lead or build checks — `SlotContext.unlimited`), there is **no work order**, no build board entry, no reminders/nudges (clock filters `kind = bike`), no storage. Parts events go to Klaviyo under their own metric names (`PARTS_METRIC` in messages.ts: Parts: Order Arrived / Booked / Rescheduled / Cancelled / Missed / Collected) so bike flows stay untouched. Handover is a single **Collected** button (`collectParts`: appointment completed, unit picked up, order fulfilled, `Pickup: Completed` with `item_kind: parts`). When Lightspeed completes the special-order line it drops out of the open list and the next sync marks the order fulfilled (`fulfilled_in_lightspeed`), so it leaves the Parts tab on its own. Bikes pages exclude `parts` everywhere.
