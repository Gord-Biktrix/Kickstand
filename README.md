...
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` | Sign in with Google; when set, staff sign in with their Workspace account and no emails are needed |
| `AUTH_EMAIL_FROM`, `RESEND_API_KEY` | Magic-link email (fallback when Google isn't configured). Console logging when the key is empty |
...
- Due date: `lightspeed.due_mode` decides what the work order's Due (`etaOut`) means. `pickup` = the customer's slot (Ikeono's `{ETA Out}` smart field can quote it). `assembly` (Vancouver) = the build deadline: `assembly_due_time_local` (10:00) on the pickup day, or the previous open day if the slot is earlier than that; the pickup time then lives in Hook Out and the note, and the Ikeono text must not use `{ETA Out}`. Hook Out reads `BUILD BY: Saturday 10:00 am (5 Sep) · Pickup Saturday 2:45 pm (5 Sep)` — build deadline first, in caps, because that is what the mechanic reads; the note lists the build line before the pickup line for the same reason, and after a reschedule adds `RESCHEDULED n×: was <old slot>[, then <older>]` so the mechanic sees the history. Set with `pnpm ls:setup … --due assembly --due-time 10:00`. **`lead` (Vancouver from 2026-09-05)** = the build deadline is `assembly_lead_work_hours` (default 8) working hours before the pickup, counting open days only and treating each as an 8-hour shop day — so 8 means the same clock time on the previous open day (Friday 12:00 pickup → Thursday 12:00 build-by). `pnpm ls:setup … --due lead --lead-hours 8`.
...
**Parts & accessories (2026-09-05):** `orders.kind` / `units.kind` = `bike` | `parts`. Parts special-order lines from Lightspeed become `parts` orders (one per line, `SO<lineID>`, listed per customer on the **Parts** tab, `/app/parts`). They are invited like bikes (one text per customer listing all items), the customer books **any future slot** (≥ 1 h ahead; no capacity, concurrency, lead or build checks — `SlotContext.unlimited`), there is **no work order**, no build board entry, no reminders/nudges (clock filters `kind = bike`), no storage. Only the arrival event differs for parts (`PARTS_METRIC`: Pickup: Bike Arrived → **Parts: Order Arrived**); Booked / Rescheduled / Cancelled / Completed share the bike flows, with templates branching on `item_kind`. Handover is a single **Collected** button (`collectParts`: appointment completed, unit picked up, order fulfilled, `Pickup: Completed` with `item_kind: parts`). When Lightspeed completes the special-order line it drops out of the open list and the next sync marks the order fulfilled (`fulfilled_in_lightspeed`), so it leaves the Parts tab on its own. Bikes pages exclude `parts` everywhere.

**Special-order sync (2026-09-05):** `src/lib/special-orders.ts` pulls Lightspeed's uncompleted special-order lines (SaleLine `isSpecialOrder`, `saleID` 0, per `settings.lightspeed.shop_id`, last 180 days) every clock tick and on the **Sync from Lightspeed** button on Bikes. Lines whose item category starts with `Bikes` become Kickstand orders (`orderRef` `SO<saleLineID>`, `ls_sale_line_id` for idempotency, customer name/phone/email from Lightspeed, model/size/colour from the item matrix, `paymentStatus` deposit + a note to confirm the balance — the line carries no payment data); parts and accessories are counted and skipped. Re-runs update changed customer/bike fields on open orders. An open order that arrived another way (CSV import, Add an order, register button) with no line id is **adopted** when the customer (Lightspeed id, phone or email) and model match, so import + sync never makes twins. Orders with no box yet show under **On order** at the foot of Bikes as a selectable table with a model dropdown and text filter: tick bikes → **Send invites** (`inviteOrders`: receives each under its sale reference as the box tag, then `inviteUnit`; skips bikes with no contact or already in the building), or Invite per row (Receive with a custom box tag lives only on the Receive a box page). Tests feed a fake `SpecialOrderSource`.

**Klaviyo quiet hours (2026-09-08):** Klaviyo held an evening test text (and the email queued behind it in the same flow) until the next morning: SMS *Quiet hours* was on for every Pickup flow and the profile had no time zone, so Klaviyo used the account's US/Eastern. Events now send `location.timezone = America/Vancouver` on the profile. Still turn **Quiet hours off** on each SMS step of the Pickup flows (they are transactional confirmations), and Smart Sending off on the Bike Arrived email.

**Passwords (2026-09-08, Gord's call after the trade-offs were laid out):** optional per account. A signed-in person sets one under **Your account** (`/app/account`, header name link); scrypt hash in `staff_users.password_hash` (`lib/passwords.ts`, min 10 chars). Login page offers Google, then email + password, with the emailed link behind "Email me a sign-in link". Five wrong tries lock the password for 15 min (`password_failed`, `password_locked_until`); Google/link still work. Changing a password signs out other devices. People without a password see a one-line banner after sign-in linking to Your account ("Not now" sets `ks_pw_nudge` for 90 days via POST /app/account/dismiss); the login form says the same. Admins can **Reset** (clear) a managed person's password from Settings › Staff; there is no admin-set password and no 2FA. Migration 0007.

**Super admin (2026-09-08):** role `owner`, label "Super admin", above admin (`ROLE_RANK`). Rules in `lib/roles.ts`: you may hand out and manage (role/store change, deactivate, delete) only roles strictly below your own (`assignableRoles`, `canManage`); nobody assigns owner from the UI and nobody can touch the owner. Admins therefore run stores and manage staff/managers; only the owner creates or removes admins. Give the role with `ENV_FILE=.env.production pnpm staff:add --email gord@biktrix.com --name Gordon --role owner --showroom all` (migration 0006 widens the role check first; deploy the code before assigning, or the owner is locked out as an unknown role). Passwords are deliberately not offered — Google Workspace is the credential.

**Logo (2026-09-08):** Kickstand "K strut" mark from Claude Design — see `docs/brand/`. `src/app/icon.svg` + `apple-icon.tsx` (ImageResponse PNG) replace the default favicon; `components/logo.tsx` renders the header and login wordmark; staff pages are titled `… · Kickstand`, customer pages stay `… · Biktrix Pickups`.

**Sign in with Google (2026-09-08):** set `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (Google Cloud → OAuth client, type Web, consent screen *Internal*, redirect URI `<APP_BASE_URL>/auth/google/callback`). `/auth/google` starts the code flow (state cookie, `hd` hint), `/auth/google/callback` exchanges the code, validates the ID token via Google's tokeninfo endpoint (`checkGoogleClaims`: aud, iss, email_verified, exp, `hd` = `AUTH_ALLOWED_DOMAIN`) and only an **active staff row** gets a session (`createSessionForEmail`). With Google on, Settings › Staff just adds the row ("Add to staff") — no invite email, no Resend needed — and the emailed link is hidden in production (still shown locally as a dev fallback).

**Staff accounts (2026-09-05):** Settings › **Staff** — invite by work email (name, role, store); `inviteStaff` upserts the `staff_users` row and emails a sign-in link valid 7 days (`sendInviteEmail`, Resend or console). Resend link, change role/store, deactivate (revokes every session) / re-activate, or delete (removes sessions, pending links and the row; timeline entries keep the recorded name). Managers invite staff into their own store; only admins create admins, change roles or deactivate. `AUTH_ALLOWED_EMAILS` is now only a bootstrap list — anyone with an active staff row may sign in as long as the domain matches. `pnpm staff:add` still works for scripting.
...
**Bike page:** leads with the Pickup card — the booked slot with Build / Ready / Start handover / Reschedule / Cancel booking beside it (or Send invite / Book for customer when unbooked), then the customer, then dates and storage folded away. Staff cancel (`staffCancelBookingAction`) offers three reasons: *customer asked* (R8 cutoff applies, customer texted a rebook link), *shop* (we had to cancel — customer texted a rebook link with `cancelled_by: shop` on the `Pickup: Cancelled` event so the Klaviyo flow can word it as an apology; never a no-show) or *staff* (mistake — silent, never a no-show). **One visit, several bikes (2026-09-05, was P1):** `appointments.group_id` links the bikes a customer collects together. `bookGroup` books each bike at the same time under one group — capacity counts every bike (build time is per bike), the same-time concurrency check ignores group siblings, and one `Pickup: Booked` message goes out with `bike_count` / `bikes`. Cancel, reschedule and no-show act on the whole visit (one message each); the day-before reminder is one per visit. The customer's booking page and the staff slot picker list the person's other bookable bikes under **Collect together**, ticked by default (`bookableSiblings`; availability is computed for the visit size). `inviteUnits` groups invites per customer (one "your N bikes have arrived" text); when the customer already has a future visit and the bike can still be built in time it **joins** that visit (`joined_existing_pickup`, `slot_start_local` on the Bike Arrived event) instead of asking for another booking. Today shows one card per visit with a handover button per bike; the bike page lists "Same visit" mates.

**Back to On order:** on a received or invited bike with no booking, the bike page offers *Back to On order* (`unreceiveUnit`): the unit and its unit-level events are removed, the order stays open (so it reappears under On order and the special-order sync re-links it), a `unit_unreceived` event is logged on the order, nothing is sent. **Delete (manager):** *Delete this bike* on the bike page, or *Delete* in the bulk bar, hard-deletes a bike that should never have existed (test, duplicate, wrong customer): unit, appointments and events go, the order too when it has no other unit, and any booked day's counter is freed (`deleteUnit`). A single `unit_deleted` event (no unit/order id) records who, why and what. Nothing is sent to the customer; Lightspeed work orders are not touched. **Bulk actions:** tick bikes on the Bikes page (checkboxes bound to one form via the `form` attribute) and use the bar above the table — Send invites, Mark building, Mark ready, Cancel bookings with a reason (`bulkBikesAction`; each bike is processed independently and the flash lists skips). The Appointments week view links each day's count to `/app/bikes?filter=booked&date=…` so a closed day can be cancelled in one go. Staff reschedule reuses the slot picker at `/app/book?unit=…&reschedule=1` and `rescheduleBooking` (customer-requested semantics).
